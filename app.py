    ...
    try:
        with open(path, "r") as f:
            ...
            if isinstance(tokens, list) and all(isinstance(t, dict) and "token" in t for t in tokens):
                ...
                print(f"Warning: Token file {path} is not in the expected format.")
                return []
    except FileNotFoundError:
        print(f"Warning: Token file {path} not found.")
        return []
    except json.JSONDecodeError:
        print(f"Warning: Token file {path} contains invalid JSON.")
        return []
...
async def send_likes_with_token_batch(uid, server_region_for_like_proto, like_api_url, token_batch_to_use):
    if not token_batch_to_use:
        print("No tokens provided in the batch.")
        return []
...
def make_profile_check_request(encrypted_profile_payload, server_name, token_dict):
    ...
    try:
        ...
        print(f"HTTP error for token {token_value[:10]}...: {e.response.status_code}")
    except requests.exceptions.RequestException as e:
        print(f"Request error for token {token_value[:10]}...: {e}")
    except Exception as e:
        print(f"Unexpected error for token {token_value[:10]}...: {e}")
    return None

def make_profile_check_request_with_fallback(encrypted_profile_payload, server_name, all_tokens, max_attempts=3):
    """Try up to max_attempts tokens for profile check, return (info, tokens_used_count)"""
    tokens_tried_count = 0

    for i in range(min(max_attempts, len(all_tokens))):
        token_dict = all_tokens[i]
        tokens_tried_count += 1
        result = make_profile_check_request(encrypted_profile_payload, server_name, token_dict)
        if result and hasattr(result, 'AccountInfo'):
            print(f"Profile check succeeded with token index {i}")
            return result, tokens_tried_count
        print(f"Profile check failed with token index {i}, trying next...")

    print(f"All {max_attempts} token attempts failed for profile check.")
    return None, tokens_tried_count
...
def handle_requests():
    ...
    # Get likes BEFORE with fallback (tries up to 3 tokens)
    before_info, tokens_used_for_before = make_profile_check_request_with_fallback(
        encrypted_player_uid_for_profile, server_name_param, all_available_tokens, max_attempts=3
    )
    before_like_count = 0
    ...
    if before_info and hasattr(before_info, 'AccountInfo'):
        ...
        print(f"Could not fetch 'before' profile info for UID {uid_param}.")
    ...
    if tokens_for_like_sending:
        print(f"Sending likes for {server_name_param} with batch size {len(tokens_for_like_sending)}.")
        ...
        print(f"Skipping like sending for UID {uid_param} — no tokens available.")

    # Get likes AFTER with fallback (tries up to 3 tokens)
    after_info, tokens_used_for_after = make_profile_check_request_with_fallback(
        encrypted_player_uid_for_profile, server_name_param, all_available_tokens, max_attempts=3
    )
    ...
    if after_info and hasattr(after_info, 'AccountInfo'):
        ...
        player_nickname_from_profile = (
            str(after_info.AccountInfo.PlayerNickname)
            if after_info.AccountInfo.PlayerNickname else "N/A"
        )
    else:
        print(f"Could not fetch 'after' profile info for UID {uid_param}.")
    ...
    response_data = {
        ...
        "TokensUsedForProfileCheck": {
            "before": tokens_used_for_before,
            "after": tokens_used_for_after
        },
        "LikeBatchSize": len(tokens_for_like_sending),
        "BatchMode": "random" if use_random else "rotating",
        "Note": (
            f"Profile check used up to 3 token fallbacks. "
            f"{'Random' if use_random else 'Rotating'} batch of {len(tokens_for_like_sending)} tokens used for likes."
        ),
        "dev": "@viswajeethu"
    }
